...
namespace Sonata.Diagnostics.Logging.Appenders
{
	...
	public class AdoNetAppender : BufferingAppenderSkeleton
	{
		...
		#region BufferingAppenderSkeleton Members
		...
		protected override void SendBuffer(LoggingEvent[] events)
		{
			...
			if (UseTransactions)
			{
				...
				using (var dbTran = Connection.BeginTransaction())
				{
					try
					{
		...
		protected virtual Type ResolveConnectionType()
		{
			try
			{
				return SystemInfo.GetTypeFromString(Assembly.GetCallingAssembly(), ConnectionType, true, false);
			}
			catch (Exception ex)
			{
		...
		private void InitializeDatabaseConnection()
		{
			...
			var resolvedConnectionString = string.Empty;
	...
	public class AdoNetAppenderParameter
	{
		...
		public virtual void FormatValue(IDbCommand command, LoggingEvent loggingEvent)
		{
			...
			// Format the value
			var formattedValue = Layout.Format(loggingEvent) ?? DBNull.Value;
